...
# The following lines from NXP are disabled, since they break building lightweigth-sdk.
# For PTIC there is no reason to switch from rpm (the yocto default) to deb, or have a package manager.
...
echo "BBLAYERS += \"\${BSPDIR}/sources/meta-ptic\"" >> $BUILD_DIR/conf/bblayers.conf
echo "BBLAYERS += \"\${BSPDIR}/sources/meta-ptic-signing-tool\"" >> $BUILD_DIR/conf/bblayers.conf
echo "BBLAYERS += \"\${BSPDIR}/sources/meta-ptic-emmc-retention-guard\"" >> $BUILD_DIR/conf/bblayers.conf

# Add the virtualization layer when docker (or other virtualization solutions) are required. when not required, adding
# the layer will just throw an annoying warning.
